ActiveAdmin.register Episode do
  permit_params :id, :video_id, :title, :description, :published_at, :image1, :image2, :image3, :tags_listing, :sort_order, :active, video_organizations_attributes: [:id, :organization_id, :_destroy],
    ...
    tabs do
      tab 'Video' do
        if episode.video_id.present?
          ...
          f.inputs :tags_listing, label: 'Tags'
          f.inputs do
            f.input :active
          end
        else
